...
Live: **https://sumeetm10.github.io/wired-future/**

Three things must all be right for that to happen, and each is easy to get wrong:

1. **An origin trial token for the exact origin.** WebMCP runs as a Chrome origin trial
   through Chrome 156. Without a token the API simply does not exist for your page.
2. **`http-equiv`, not `name`.** Chrome only reads `<meta http-equiv="origin-trial">`.
   Next's `metadata.other` API emits `name="origin-trial"`, which is silently ignored — the
   token ships, looks correct in view-source, and does nothing.
3. **Probe both surfaces.** The API moved from `navigator.modelContext` to
   `document.modelContext` in the 21 July 2026 revision. Checking only one misses a
   conforming runtime entirely.

For the record, the flag route was measured and does NOT work: on Chrome 152.0.7977.75 and
...
1. **ChatGPT's in-app browser** — native WebMCP support. Open the deployed URL there and ask it
   to change the scene.
2. **Plain Chrome on the deployed URL** — this works today, because the origin trial token
   is served. Set `NEXT_PUBLIC_ORIGIN_TRIAL_TOKEN` locally, or add an `ORIGIN_TRIAL_TOKEN`
   repo secret for CI. See Status above for the three things that must line up.
